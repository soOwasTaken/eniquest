<template>
  <div class="firefly-container">
    <div v-for="(firefly, index) in numberOfFireflies" :key="index" class="firefly" :style="fireflyStyles[index]" :ref="'firefly_' + index"></div>
  </div>
</template>
...
export default {
  ...
  methods: {
    generateInitialStyles() {
      ...
      const spawnRadius = Math.min(window.innerWidth, window.innerHeight) * 0.30; // Adjust spawn radius as needed
